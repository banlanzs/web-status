...
## 2. 90天可用率计算 (90-Day Uptime Ratio) - 严格模式
**问题**: 用户反馈在暂停监测期间，90天可用率维持不变（例如 98.62%），期望暂停期间的可用率应被视为 0（拉低整体评分）。
**修复**:
*   **严格计算逻辑**: 将暂停时长视为**不可用时间**。
*   **算法变更**: `可用率 = (总时间 - 故障时间 - 暂停时间) / 总时间`。
*   **效果**: 暂停时间虽然不计入故障次数，但会作为“0分”时段计入分母。这意味着如果监控暂停了 22 天，这 22 天的贡献度为 0%，会显著拉低 90 天的平均可用率（符合“无数据=0可用”的严格标准）。
